...
public class MarioComponent extends JComponent implements Runnable,
	...
	private void toggleKey(int keyCode, boolean isPressed) {
		...
		if (isPressed && keyCode == KeyEvent.VK_ESCAPE) {
			...
			if ((scene.getClass().getName() == "dk.itu.mario.scene.WinScene")
					|| (scene.getClass().getName() == "dk.itu.mario.scene.LoseScene")) {
				this.showFinalTestInterface();
			}
	...
	private void showFinalTestInterface() {
		Charset cod = Charset.defaultCharset();
		Path file = FileSystems.getDefault().getPath(
				System.getProperty("user.home"), "mario","avaliacao.txt");
...
//		}
		...
		try (Scanner s = new Scanner(System.in);
				BufferedWriter b = Files.newBufferedWriter(file, cod,
						StandardOpenOption.CREATE, StandardOpenOption.WRITE,
						StandardOpenOption.APPEND)) {
			System.out.println("Dê uma avaliação para essa fase [0-5]: ");
			this.levelResult = s.nextInt();

			b.write(Integer.toString(this.levelResult) + '\n');

		} catch (InputMismatchException e) {
			System.err
					.println("Você inseriu a nota incorretamente. Digite um valor inteiro entre 0 e 5!");
			this.showFinalTestInterface();
		} catch (NoSuchFileException e) {
			System.err.print("Arquivo ou diretório não encontrado: "
					+ e.getFile());
		} catch (IOException e) {
